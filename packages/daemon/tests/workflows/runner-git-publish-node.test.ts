...
`,
    ...
    const pushedCommit = await runGit(['--git-dir', remoteDir, 'rev-parse', 'refs/heads/viewport/proof'], root);
...
`,
    ...
    expect(failed?.status).toBe('failed');
    expect(failed?.nodes.publish?.error).toContain('Run-scoped git publish grant repo/github/payments-api was not materialized');
...
async function runGit(args: string[], cwd: string): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    ...
    child.once('close', (code) => {
      ...
      else reject(new Error(Buffer.concat(stderr).toString('utf8') || `git ${args.join(' ')} failed with ${code}`));
    });
  });
}
