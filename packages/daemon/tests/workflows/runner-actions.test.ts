...
`,
  ...
  it('renders Slack completion placeholders from prior GitHub receipts', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(new Response(JSON.stringify([]), { status: 200 }))
      .mockResolvedValueOnce(
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `
schema: viewport.workflow/v1
name: slack-completion-template-proof
nodes:
  open_pr:
    type: action
    adapter: github
  ...
  notify_completion:
    ...
    action: post_message
    needs: [open_pr]
    idempotencyKey: slack-completion:PAY-1842
    with:
      channel: C123
      text: "GitHub PR -> <{github_pr.url}|{github_pr.label}>\\nAudit packet -> <{audit_packet.url}|Open audit packet>\\nRun -> <{run.url}|Open run>"
...
`,
      ...
      if (originalGitHubCredentialRefToken === undefined) delete process.env[githubTokenEnv];
      else process.env[githubTokenEnv] = originalGitHubCredentialRefToken;
...
`,
  ...
  it('keeps run-scoped provider credentials available when resuming after approval', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            number: 43,
            html_url: 'https://github.com/acme/payments/pull/43',
            url: 'https://api.github.com/repos/acme/payments/pulls/43',
          }),
          { status: 201 },
        ),
      )
    ...
    await fs.writeFile(
      ...
      `
schema: viewport.workflow/v1
name: provider-credential-resume-proof
nodes:
  review:
    type: approval
  ...
  open_pr:
    ...
    adapter: github
    action: pull_request.create
    idempotencyKey: pr-resume:PAY-1843
    with:
      repository: acme/payments
      title: Viewport support fix
      head: viewport/pay-1843
      base: main
      body: "Tests are passing."
      credential_ref: github/pr-writer
`,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        headers: expect.objectContaining({
          Authorization: 'Bearer ghs_run_scoped_pr_writer_after_approval',
          'Idempotency-Key': 'pr-resume:PAY-1843',
        }),
      }),
    );
    expect(completed?.status).toBe('completed');
    expect(completed?.nodes.open_pr?.metadata?.action).toMatchObject({
      ...
      response: {
        status: 201,
        htmlUrl: 'https://github.com/acme/payments/pull/43',
        number: 43,
      },
    });
  });
