...
export class WorkflowRunner {
  ...
  /**
   * Resume runs that were `running` when the daemon last shut down. Each
   * candidate's partially-executed nodes are reset to `queued` so the next
   * scheduling pass picks them up. Runs in `blocked` state are left alone —
   * they're waiting on an external event (approval / schedule gate) and
   * resume themselves when that fires.
   *
