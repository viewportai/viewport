...
function createDeviceApprovalRequest(home, { deviceName, code, keyStore = null }) {
  const identity = createCryptoIdentity(deviceName);
  updateIdentity(home, deviceName, {
    ...identity,
    deviceName,
...
async function acceptDeviceApproval(home, { userName, deviceName, approval, code, keyStore = null }) {
  ...
  const deviceIdentity = getIdentity(home, deviceName, { keyStore });
  ...
  return getIdentity(home, deviceName, { keyStore });
}
